...
//        }
    ...
    fun testBasicPrint() {
        ...
        val result = interpreter.interpret(getTree(string), symbolTable, InputProvider(emptyList()))
    ...
    fun testDeclareVariableAndThenPrintIt() {
        val string = "let num: number = 3;"
        interpreter.interpret(getTree(string), symbolTable, InputProvider(emptyList()))
        val string2 = "println(num);"
        val result = interpreter.interpret(getTree(string2), symbolTable, InputProvider(emptyList()))
        result as PrintResult
    ...
    fun testDeclareVariableOfTypeStringWithNumberLiteral() {
        ...
        assertThrows<InvalidDataTypeException> { interpreter.interpret(getTree(string), symbolTable, InputProvider(emptyList())) }
    }
    ...
    fun testDeclareVariableWithTypeStringAndThenAssignANumber() {
        ...
        assertThrows<UnsupportedOperationException> { interpreter.interpret(getTree(string2), symbolTable, InputProvider(emptyList())) }
    }
    ...
    fun testAssignDeclarationWithoutValueAndThenPrintIt() {
        ...
        val string2 = "s = 'hola';"
        interpreter.interpret(getTree(string), symbolTable, InputProvider(emptyList()))
        interpreter.interpret(getTree(string2), symbolTable, InputProvider(emptyList()))
        val string3 = "println(s);"
        val result = interpreter.interpret(getTree(string3), symbolTable, InputProvider(emptyList()))
        result as PrintResult
    ...
    fun testEnvVariableIsCorrectlyPrinted() {
        ...
        val result = interpreter.interpret(tree, symbolTable, InputProvider(emptyList()))
    ...
    fun testBooleanLiteralReturn() {
        ...
        val result = interpreter.interpret(getTree(string), symbolTable, InputProvider(emptyList())) as Result
    ...
    fun testIfNodeThatHasTrueConditional() {
        ...
        val result = interpreter.interpret(ifNode, symbolTable, InputProvider(emptyList())) as MultipleResults
    ...
    fun testIfNodeWithoutElseBlockWhenConditionIsFalse() {
        ...
        val ifNode = IfNode(literalNode, thenBlock, elseBlock, tokenPosition)
        val result: Result = interpreter.interpret(ifNode, symbolTable, InputProvider(emptyList())) as Result
    ...
    fun testBuilderReturnsCorrectVersion() {
        ...
        assertThrows<UnsupportedOperationException> { oldInterpreter.interpret(getTree(string), symbolTable, InputProvider(emptyList())) }
    }
}
